...
public:
  ...
  // Gearing
  float gearRatios[7] = {0, 0.8, 1.4, 1.6, 1.9, 2.4, 2.8};
  float gearLazyValues[7] = {0.99,   0.999,  0.9995, 0.9996,
                             0.9997, 0.9998, 0.9999};
  float gearThrottleResponses[7] = {1, 0.1, 0.06, 0.05, 0.04, 0.035, 0.03};

  // Engine
  double lazyValue = 0.99; // Engine time for revs to settle back - values
                           // closer to one need more time to go back to idle
  double throttleResponse =
      1; // Throttle sensitivity - Should feel lower in higher gears since we
         // reduce the value after every upshift bruh

  // Wheel
  double coastLazyValue =
      0.999;             // Driving drag on wheels (and also engine if in gear)
  float brakeFactor = 1; // Brake Drag on wheels (and also engine if in gear) -
                         // Basically the same as coastLazyValue
  ...
  void tick() {
    while (running) {
      ...
      // Apply clutch revs in multiple smaller chunks of revs, makes a kick,
      // like dumping the clutch in a real car Idk how this works
      rpm += clutch * clutchKick;
      clutch = (1.0 - clutchKick) * clutch;

      // Wheel RPM depending on the engine rpm , current gear ratio and coasting
      // drag
      if (gear >= 1) {
  ...
  void setGear(int newGear) {
    ...
    if (gear > 0) {
      ...
      if (wheelRPM <= 0) {
        clutch = 500 - rpm; // rpm + clutch would be 500 (since clutch takes a
                            // rev difference that it smoothly applies)
        return;
      }
    }
  }
  ...
  void setWheelSpeed(float newSpeed) {
    // Sets wheelRPM for next tick
    wheelRPM = newSpeed;
  }
